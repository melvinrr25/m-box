(function() {
  ...
  function buildButtons(button, index, boxButtonsArea) {
    var newBtn = document.createElement("button");
    newBtn.classList.add("mbx-button-" + (index + 1));
    newBtn.innerHTML = button.hasOwnProperty("label") ? button.label : "OK";

    if (button.hasOwnProperty("class")) {
      newBtn.classList.add(button.class);
    } else {
      newBtn.setAttribute("style", strings.buttons);
    }

    for(var style in button.styles){
      newBtn.style[style] = button.styles[style];
    }

    if(button.hasOwnProperty("background")){
      newBtn.style.background = button.background;
    }

    if(button.hasOwnProperty("color")){
      newBtn.style.color = button.color;
    }
    ...
    boxButtonsArea.appendChild(newBtn);
    newBtn.focus();
  }
    ...
    display: function(settings) {
      if (Object.prototype.toString.call(settings) === "[object Object]") {
        ...
        boxBase.addEventListener("click", function(e) {
          if (
            e.target.getAttribute("class") === "mbx-base" &&
            settings.preventOutsideClick === false
          ) {
            mbx.close();
          }
        ...
        if (settings.hasOwnProperty("buttons")) {
          if (settings.buttons instanceof Array) {
            settings.buttons.forEach(function(button, index) {
              buildButtons(button, index, boxButtonsArea);
            });
          }
        } else {
